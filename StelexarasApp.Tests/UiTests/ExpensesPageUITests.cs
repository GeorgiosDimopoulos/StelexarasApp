namespace StelexarasApp.Tests.UiTests
{
    ...
    namespace StelexarasApp.Tests.UITests
    {
        public class ExpensesPageUITests : IDisposable
        {
            private readonly IWebDriver _driver;

            public ExpensesPageUITests()
            {
            ...
            public void CreatePage_ShouldLoadSuccessfully()
            {
                ...
                // Assert
                Assert.Equal("Create Expenses - StelexarasApp", pageTitle);
            }
            ...
            {
                // Arrange
                _driver.Navigate().GoToUrl("http://localhost:5000/Expenses/Index");

                // Act
                var pageTitle = _driver.Title;

                // Assert
                Assert.Equal("Expenses - StelexarasApp", pageTitle);
            }

            [Fact(Skip = "Temporarily ignoring this test")]
            public void CreatePage_ShouldCreateNewExpense()
            {
                // Arrange
                _driver.Navigate().GoToUrl("http://localhost:5000/Duties/Create");

                // Act
                _driver.FindElement(By.Id("Name")).SendKeys("Test Expenses");
